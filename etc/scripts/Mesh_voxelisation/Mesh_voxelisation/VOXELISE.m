...
if narginremaining==4
  ...
  if isstruct(varargin{1}) || ishandle(varargin{1})

      if isfield(varargin{1},'vertices') % added by peres.asc 2022.07.12
          meshXYZ = CONVERT_meshformat(varargin{1}.faces,varargin{1}.vertices);
      else
          meshXYZ = CONVERT_meshformat(varargin{1}.Faces,varargin{1}.Vertices); % changed faces by Faces and vertices by Vertices
      end
