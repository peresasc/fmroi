    function [P, J, J1] = regiongrowingmask(cIM, initPos, thresVal, maxDist, tfMean, tfFillHoles, tfSimplify, grwmeth, clustersize,mask)
% REGIONGROWING Region growing algorithm for 2D/3D grayscale images
%
% Syntax:
%   P = regiongrowingmask();
%   P = regiongrowingmask(cIM);
%   P = regiongrowingmask(cIM, initPos)
%   P = regiongrowingmask(..., thresVal, maxDist, tfMean, tfFillHoles, tfSimpl)
%   [P, J] = regiongrowingmask(...);
%
% Inputs:
...
%
%  *** Added by Peres ASC in 2019/12/03 ***
%
%      grwmeth: Growing method - search for most similar neighbor {diff} or
%               search for the maximum neighbor {max}
%  clustersize: Max number of elements {Inf}
%         mask: The algorith will perform just in the region defined by the
%               mask
%  *** END ***
%
% Outputs:
...
%   poly = regiongrowingmask(cIM, [], 300); % click somewhere inside the lungs
%   plot(poly(:,1), poly(:,2), 'LineWidth', 2)
%   
%   % 3D Example
%   load mri
%   poly = regiongrowingmask(squeeze(D), [66,55,13], 60, Inf, [], true, false);
...
%--------------------------------------------------------------------------
% MUDAR ESSA ESTRATEGIA DE LER IMAGEM
...
if ~exist('initPos', 'var') || isempty(initPos)
    himage = findobj('Type', 'image'); %    MUDAR
    if isempty(himage)
        himage = imshow(cIM, []);
    end
...
if initPos(1) < 1 || initPos(2) < 1 ||...
   initPos(1) > nRow || initPos(2) > nCol % ACRESCENTAR O LIMITE PARA OS SLICES
    error('Initial position out of bounds, please try again!')
end

%--------------------------------------------------------------------------
% REVER ESSES VALORES DE THRESHOLD
...
if ~exist('grwmeth', 'var') || isempty(grwmeth)
    grwmeth = 'diff';
end

if ~exist('clustersize', 'var') || isempty(clustersize)
    clustersize = Inf;
end

if ~exist('clustersize', 'var') || isempty(mask)
    mask = ones(size(cIM));
end
...
% defining the region to apply regionGroing
vetmask = find(mask);
...
queue = [initPos(1), initPos(2), initPos(3), regVal];
ncluster = 0;

%%% START OF REGION GROWING ALGORITHM
while size(queue, 1)
  % the first queue position determines the new values
  if strcmpi(grwmeth,'diff') % added by Peres ASC
      qdiff = abs(queue(:,4)-regVal);
      [~,qpos] = min(qdiff);
  elseif strcmpi(grwmeth,'max') % added by Peres ASC
      [~,qpos] = max(queue(:,4));
  else
      error('Selected method not specified. Allowed entry: {max} or {diff}.')
  end
  ...
  % .. and delete the caught queue position
  queue(qpos,:) = [];
    ...
    for j = -1:1
      for k = -1:1
        ...
        if xv+i > 0  &&  xv+i <= nRow &&...          % within the x-bounds?
           ...
           cIM(xv+i, yv+j, zv+k) >= (regVal - thresVal) &&...% of the thre?
           ncluster<clustersize &&...          % within the nvoxels-bounds?
           ismember(sub2ind(size(cIM),xv+i,yv+j,zv+k),vetmask) % within the mask-bounds?
           ...
           queue(end+1,:) = [xv+i, yv+j, zv+k, cIM(xv+i, yv+j, zv+k)];
