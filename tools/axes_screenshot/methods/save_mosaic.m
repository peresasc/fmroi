function save_mosaic(hObject)
% save_mosaic is an internal function of fMROI. It saves the mosaic image
% generated by the mosaic function. For a comprehensive user guide, please
% access the documentation webpage:
...
if isfield(handles,'mosaicax')
    if isobject(handles.mosaicax(1))
        if isvalid(handles.mosaicax(1))
            ...
            pos = [];
            for i = 1:numel(handles.mosaicax)
                p = GetLayoutInformation(handles.mosaicax(i));
                pos = [pos;p.PlotBox];
            end
            x = min(pos(:,1));
            y = min(pos(:,2));
            ...
            outfn = fullfile(pn,[fn,'_mosaic',ax_tag,ext]);
            imwrite(im,outfn)
